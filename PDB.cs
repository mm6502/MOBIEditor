	...
	public class PDBFormat {
		...
		public PDBFormat(FileInfo pdbFile) {
			...
			using (BinaryReader2 bin = new BinaryReader2(pdbFile.OpenRead())) {
				...
				for (short i = 0; i < NumRecords; i++) {
					PDBRecordInfo rec = RecordInfos[i];
					int dataEnd = (i + 1 < NumRecords ? RecordInfos[i + 1].DataOffset : (int)bin.BaseStream.Length);
		...
		public void Save(FileInfo pdbFile) {
			using (BinaryWriter2 bin = new BinaryWriter2(pdbFile.OpenWrite(), Encoding.ASCII)) {
				...
				int nextOffset = 78 + NumRecords * 8 + GapToData.Length; // this may also have to include the appinfo and sortinfo byte[]
				for (short i = 0; i < NumRecords; i++) {
					PDBRecordInfo rec = RecordInfos[i];
					...
					byte[] temp = BitConverter.GetBytes(rec.UniqueId);
					Array.Reverse(temp);
					bin.Write(temp, 1, 3);
