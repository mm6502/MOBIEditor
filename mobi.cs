	...
	public class MOBIFormat {
		...
		public class MobiHeaderFormat {
			...
			public int GetInt32_BigEndian(int offset) {
				return MOBIFormat.Get_BigEndian_Int32_FromByteArray(Data, offset);
			}
		}
		...
		public MOBIFormat(FileInfo mobiFile) {
			...
			PDBFormat.PDBRecordInfo firstRecord = PDB.RecordInfos.First();

			using (BinaryReader2 bin = new BinaryReader2(new MemoryStream(firstRecord.Data))) {
				// create PalmDoc header
				PalmDocHeader = new PalmDocHeaderFormat();
				PalmDocHeader.Compression = (PalmDocHeaderFormat.CompressionType)bin.ReadInt16_BigEndian();
				...
				if (MobiHeader.HasEXTH) {
					...
					for (int i = 0; i < numRecords; i++) {
						ExthHeaderFormat.ExthHeaderRecordFormat rec = new ExthHeaderFormat.ExthHeaderRecordFormat();
						rec.Type = bin.ReadInt32_BigEndian();
						int length = bin.ReadInt32_BigEndian();
						rec.Data = bin.ReadBytes(length - 8);
						ExthHeader.Records.Add(rec);
					}
					for (int p = 0; p < ExthHeader.Padding; p++) bin.ReadByte(); // consume and ignore the padding
				}
		...
		public void Save(FileInfo mobiFile) {
			...
			BinaryWriter2 bin = new BinaryWriter2(newData);
			...
			if (MobiHeader.HasEXTH) {
				...
				for (int p = 0; p < ExthHeader.Padding; p++) bin.Write((byte)0);
			}
